...
mkdir /tmp/$sitepath
cd /tmp/$sitepath
wget --spider --recursive --level=inf --no-verbose --output-file=linklist.txt -R xml,inc,old,save,txt,js,LCK,zip,ZIP,bmp,BMP,jpg,jpeg,JPG,GIF,PNG,png,gif,CSV,csv,css,class,jar,txt,ico,svg,SVG $protocol://$sitedomain/
...
cp $name.xml $name.xml.gz $name.txt $output
rm -rf /tmp/$sitepath
